...
//  /home/elikla/.pico-sdk/tools/2.1.1/pioasm/pioasm image.pio > image.pio.h
#include "image.pio.h"
//#include <stdio.h>
...
void ov2640_init(struct ov2640_config *config) {
	...
	pwm_set_wrap(slice_num, 4);   //PWM freq = 120MHz / (wrap + 1) = 24MHz
//	pwm_set_wrap(slice_num, 11);   //PWM freq = 120MHz / (wrap + 1) = 10MHz
	...
	gpio_pull_up(config->pin_sioc);  // +2K7 pullup external
  gpio_pull_up(config->pin_siod);  // +2K7 pullup external

	// Initialise reset pin
	gpio_init(config->pin_resetb);
	gpio_set_dir(config->pin_resetb, GPIO_OUT);

	// Configura todos os pinos usados pelo PIO como entrada e função SIO
	pio_gpio_init(config->pio, CAM_PCLK); // PCLK 26
	//gpio_set_function(CAM_PCLK, GPIO_FUNC_SIO);
	//gpio_set_dir(CAM_PCLK, GPIO_IN);

	pio_gpio_init(config->pio, CAM_HREF); // HREF 21
	//gpio_set_function(CAM_HREF, GPIO_FUNC_SIO);
	//gpio_set_dir(CAM_HREF, GPIO_IN);

	pio_gpio_init(config->pio, CAM_VSYNC); // VSYNC 22
	//gpio_set_function(CAM_VSYNC, GPIO_FUNC_SIO);
	gpio_set_dir(CAM_VSYNC, GPIO_IN);

	for (int pin = CAM_D0; pin <= CAM_D7; pin++) 
	{
			pio_gpio_init(config->pio, pin); // D0..D7  12..19
  ...
  gpio_put(CAM_PWDN, 0);  //0=cam active
	...
	ov2640_regs_write(config, OV7670_rgb); // set format OV7670_yuv // OV7670_rgb
	...
	// Enable image RX PIO
	config->pio_offset = pio_add_program(config->pio, &image_program);

	image_program_init(config->pio, config->pio_sm, config->pio_offset, config->pin_y2_pio_base);

  pio_sm_set_enabled(config->pio, config->pio_sm, false);  //disable pio 



	dma_channel_config cfg = dma_channel_get_default_config(config->dma_channel);
	channel_config_set_read_increment(&cfg, false);
	channel_config_set_write_increment(&cfg, true);
	channel_config_set_dreq(&cfg, pio_get_dreq(config->pio, config->pio_sm, false));
	channel_config_set_transfer_data_size(&cfg, DMA_SIZE_16);
	
	dma_channel_configure(
		config->dma_channel, 
		&cfg,
		config->image_buf,                       //dst
		&config->pio->rxf[config->pio_sm],       //src
		config->image_buf_size,                //transfer_count
		false                          //do not start now
	);
}
...
void ov2640_capture_frame(struct ov2640_config *config) 
{
  ...
  dma_channel_abort(config->dma_channel);   //abort any DMA ativity
  dma_channel_set_write_addr(config->dma_channel, config->image_buf, false);  //refresh the dst start address
	dma_channel_start(config->dma_channel);   //restart the DMA to ready to get the RX FIFO
	pio_sm_set_enabled(config->pio, config->pio_sm, true);  //enable pio to get the data from the camera and put to RX FIFO
	dma_channel_wait_for_finish_blocking(config->dma_channel);  //wait DMA finishes the frame transfer
  pio_sm_set_enabled(config->pio, config->pio_sm, false); //disable pio (it is in a infinit loop)
  pio_sm_clear_fifos(config->pio, config->pio_sm);  //clear any extra data on fifo
	pio_sm_restart(config->pio, config->pio_sm);  //restart pio internal registers
	pio_sm_exec(config->pio, config->pio_sm, pio_encode_jmp(config->pio_offset)); // put the pio on the first instruction
...
void ov2640_regs_write(struct ov2640_config *config, const uint8_t (*regs_list)[2]) {
	...
	while (1) {
		...
		count++;
		sleep_ms(1);
		if(count>10000) { Serial.println("ov2640_regs_write"); break; }
	}
}
