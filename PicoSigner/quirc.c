...
#define TFT_HEIGHT 320  //240
	...
	#define  num_vars  (TFT_WIDTH * 2 / 3)
	struct quirc_flood_fill_vars vars[num_vars];


int quirc_resize(struct quirc *q, int w, int h, uint8_t * image)
{
//	uint8_t		*image  = NULL;
	...
	//struct quirc_flood_fill_vars *vars = (struct quirc_flood_fill_vars *) (image + (w * h));
	 ...
	 */
//	image = calloc(w, h);  //it will use the rgb_image area for the quirc
//	if (!image)
//		goto fail;
...
*/
	...
	q->w = w;
	q->h = h;
	//free(q->image);  //libera uma anterior
	q->image = image;
	#if (QUIRC_PIXEL_ALIAS_IMAGE == 0) 
	{
		free(q->pixels);  //libera uma anterior
		q->pixels = pixels;
		not used on this project
	}
	#endif
	#if (QUIRC_PIXEL_ALIAS_IMAGE == 1) 
		q->pixels = image;  //done later on pixels_setup()
	#endif
	//free(q->flood_fill_vars);  //libera uma anterior
