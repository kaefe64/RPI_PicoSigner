...
#ifdef USE_STACK
#define DECODE_RETURN(x)   return x
#endif
#ifdef USE_HEAP
#define DECODE_RETURN(x)   do{ free(pds); return x; }while(1==0)
#endif
				  ...
				  struct quirc_data *data)
{
	quirc_decode_error_t err;
#ifdef USE_STACK
	struct datastream ds;  //don't use stack 8k is too much
#endif
#ifdef USE_HEAP
	//stack size is only 2k (https://petewarden.com/2024/01/16/understanding-the-raspberry-pi-picos-memory-layout/)
	struct datastream *pds = malloc(sizeof(*pds));
	if (!pds)
	  {
			//Serial.println("quirc_decode pds malloc error ");
			return QUIRC_ERROR_DATA_OVERFLOW;
		}
#endif
	if (code->size > QUIRC_MAX_GRID_SIZE)
		DECODE_RETURN(QUIRC_ERROR_INVALID_GRID_SIZE);
	...
	if (data->version < 1 ||
		...
		DECODE_RETURN(QUIRC_ERROR_INVALID_VERSION);
...
#endif
#ifdef USE_HEAP
	read_data(code, data, pds);
	err = codestream_ecc(data, pds);
#endif
	if (err)
		DECODE_RETURN(err);
...
#endif
#ifdef USE_HEAP
	pds->raw = NULL; /* We've done with this buffer. */
#endif
...
#endif
#ifdef USE_HEAP
	err = decode_payload(data, pds);
#endif
	if (err)
		DECODE_RETURN(err);

	DECODE_RETURN(QUIRC_SUCCESS);
}
