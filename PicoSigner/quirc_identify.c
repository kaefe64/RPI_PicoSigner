...
void quirc_end(struct quirc *q)
{
	...
	uint8_t threshold = otsu(q);  // look for threshold, 256 bytes on stack
	pixels_setup(q, threshold);  //convert image byte to black white and put over image(on pixels)

	for (i = 0; i < q->h; i++)
		finder_scan(q, i);  //few local variables (calculate num_capstones)

	for (i = 0; i < q->num_capstones; i++)
		test_grouping(q, i);  //call some subs and use some locals
}
